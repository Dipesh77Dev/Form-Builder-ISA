...
module.exports = {
    getForm: async (req, res) => {  
        try{
            const getForm = await formModel.find().lean()
            res.send(getForm);
        }
        catch(err){
            res.send(err);
    ...
    getAllFormsOfUser: async(req, res)=>{
        try {
            var userId = req.params.userId;
            console.log(userId);
            await userRegisterModel.findOne({_id:userId}).then(async(user)=>{
                if(user == null){
    ...
    submitResponse : async(req, res)=>{
        try {
            ...
            if (data.response.length > 0) {
                var newResponse = new resultModel(data)
    ...
    getResponse: async(req, res)=>{
        try {
            var formId = req.params.formId;
         //   console.log(formId);
            await resultModel.find({formId: formId}).then(async(responses)=>{ 
    ...
    allResponses : async(req,res)=>{
        try{
            var result = await resultModel.find().lean();
            res.json(result);     
        }catch(e){
            res.send(e);
        }
